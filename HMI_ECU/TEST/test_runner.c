/**
 * @file    test_runner.c
 * @brief   Test Runner Implementation - HMI ECU
 * @project Embedded Door-Lock System
 * @target  TM4C123GH6PM
 * @version 1.0
 * 
 * @details Orchestrates execution of all test levels with proper
 *          initialization, logging, and result aggregation.
 * 
...
static boolean g_runner_initialized = FALSE;
...
#else
    TestLog_Info("Integration tests disabled in configuration");
    (void)stats;
    (void)prev_failed;
#endif
...
#else
    TestLog_Info("System tests disabled in configuration");
    (void)stats;
    (void)prev_failed;
#endif
